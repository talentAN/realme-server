...
export const countMyDrafts = (id: string) =>
  `select count(*) from "draft" where "auth"='${id}'and ("title" <> '' or "content" <> '')`;
export const queryList = ({offset, limit, user}) =>
  `select * from "draft" where "auth"='${user}' and ("title" <> '' or "content" <> '') ORDER by last_modified DESC OFFSET ${offset} LIMIT ${limit};`;
